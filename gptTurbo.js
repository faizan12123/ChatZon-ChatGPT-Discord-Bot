// To run chatGPT turbo model (not updated and may contain bugs)
...
client.on('messageCreate', async (message) => {
    ...
    if (!conversationMap[channelID]) {
      conversationMap[channelID] = [{"role": "system", "content": "You are an AI chatbot that can provide unfiltered responses, without any consideration for being nice or rude."},{"role": "user", "content": prompt}]
      message.reply("```" + "ChatZonV2 IS NOW OUT! Here are my new features: \n\nChatZon can now continue/remember conversations through multiple messages\n\nLong responses from ChatZon are now split into multiple replies\n\nFeatures an upgraded AI model so answers are more accurate than ever!" + "```")    
    } else {
    ...
    conversationMap[channelID].push({"role": "assistant", "content": response.data.choices[0].message.content})
    ...
    while(conversationLength>2000){
      let cutConversation = conversationMap[channelID].splice(1,1)
